...
def src_tgt_from_dict(file, border, beta):
...
def voi_nmi_from_dict(file, border, beta):
    key = key_from_border_beta(border, beta)
    voi = file[key + "voi"]
    nmi = file[key + "nmi"]
...
def tgt_output_from_dict(file, border, beta):
    key = key_from_border_beta(border, beta)
    tgt = file[key + "tgt"]
    output = file[key + "out"]
...
def get_dataset(N: int, beta_count: int, samples: int, set_size: int):
    file = np.load(
        f"data/pma/dataset__N_{N}__beta_{beta_count}__set_{set_size}__samples_{samples}.npz"
    )
    return file
...
def get_permuted_dataset(N: int, beta_count: int, samples: int, set_size: int):
    file = np.load(
        f"data/pma/dataset_permuted__N_{N}__beta_{beta_count}__set_{set_size}__samples_{samples}.npz"
    )
    return file
